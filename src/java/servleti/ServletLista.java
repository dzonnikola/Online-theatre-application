...
public class ServletLista extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            String user = "root";
            String pass = "";
            ArrayList<Film> la = new ArrayList<>();
            try {
                ...
                if (ispis.length() > 0 && ispis != null) {
                    if (ispis.equals("listaa")) {
                        String upit = "select * from film";
                        rs = st.executeQuery(upit);
                        while (rs.next()) {
                            la.add(new Film(rs.getString("filmID"), rs.getString("godina"), rs.getString("naziv"), rs.getString("trajanje"), rs.getString("opis")));
                            
                        }
                        request.setAttribute("pom", la);
